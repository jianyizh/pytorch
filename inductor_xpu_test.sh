...
rm -f ${LOG_DIR}/kernel_efficiency_${MODE}.csv
for dir in ${LOG_DIR}/${MODE}/*; do
  if [ -d "$dir" ] && [ -f "$dir/kernel.py" ]; then
    ZE_AFFINITY_MASK=0 python "$dir/kernel.py" -k | tee -a ${LOG_DIR}/kernel_efficiency_${MODE}.csv
  fi
done
sed -i '/^(I):/d' ${LOG_DIR}/kernel_efficiency_${MODE}.csv
