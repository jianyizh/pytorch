...
| `$RUN_DIR/06_kernel_arithmetic_intensity.json` | Step 6 | total_bytes |
| `$RUN_DIR/07_kernel_memory_compute_bound.json` | Step 7 | peak_bw_gbps |
| `$RUN_DIR/08_memory_load_store.json` | Step 8 | T_mem_ms, peak_bw_gbps, measured bytes |

Read ALL prior JSON files. This step MUST run after Step 8.
...
### 4. Compare with T_mem from Step 8
...
### `$RUN_DIR/09_instructions_measurement.json`

```json
{
  "step": "09_instructions_measurement",
...
### `$RUN_DIR/09_instructions_measurement.log`
...
```bash
test -f $RUN_DIR/09_instructions_measurement.json && echo "JSON OK" || echo "JSON MISSING"
python3 -c "
import json
d = json.load(open('$RUN_DIR/09_instructions_measurement.json'))
